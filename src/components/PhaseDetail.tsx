...
import { Phase, UploadedFile } from "@/types";
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          {files.map((file) => (
            <div
              key={file.id}
              className="flex items-center gap-3 bg-slate-50 rounded-md px-3 py-2 text-sm"
            >
              <svg
      ...
      <div className="space-y-8">
        ...
        <div className={`${c.bg} ${c.border} border rounded-xl p-6`}>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              <span
                className={`${c.number} w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold`}
              >
                {displayPhase.order}
              </span>
        ...
        <section>
          ...
          <div className="bg-white rounded-lg border border-slate-200 divide-y divide-slate-100">
            {(isEditing ? editApproach : displayPhase.approach).map(
              (step, i) => (
                <div key={i} className="flex items-start gap-3 p-4">
                  <span
                    className={`${c.badge} w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5`}
                  >
                    {i + 1}
                  </span>
                  {isEditing ? (
                    <div className="flex-1 flex gap-2">
                      <textarea
                        value={step}
                        onChange={(e) => {
                          const updated = [...editApproach];
                          updated[i] = e.target.value;
                          setEditApproach(updated);
                        }}
                      ...
                      <button
                        onClick={() =>
                          setEditApproach(editApproach.filter((_, j) => j !== i))
                        }
            ...
            {isEditing && (
              <div className="p-4">
                <button
                  onClick={() => setEditApproach([...editApproach, ""])}
        ...
        <section>
          ...
          <div className="grid gap-4">
            {displayPhase.deliverables.map((deliverable, i) => (
              <div
                key={i}
        ...
        <section>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-5">
            <ul className="space-y-3">
              {(isEditing ? editCautions : displayPhase.cautions).map(
                (caution, i) => (
                  <li
                    key={i}
                    ...
                    {isEditing ? (
                      <div className="flex-1 flex gap-2">
                        <textarea
                          value={caution}
                          onChange={(e) => {
                            const updated = [...editCautions];
                            updated[i] = e.target.value;
                            setEditCautions(updated);
                          }}
            ...
            {isEditing && (
              <button
                onClick={() => setEditCautions([...editCautions, ""])}
                className="text-sm text-amber-600 hover:text-amber-800 transition-colors mt-3"
              >
                + 注意事項を追加
              </button>
            )}
        ...
        {isEditing && (
          <div className="sticky bottom-4 bg-white border border-slate-200 rounded-xl shadow-lg p-4 flex items-center justify-end gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={isSaving}
              className={`${c.button} px-6 py-2 text-sm rounded-lg transition-colors disabled:opacity-50`}
            >
              {isSaving ? "保存中..." : "保存"}
            </button>
          </div>
